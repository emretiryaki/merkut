...
func (e *AlertingService) alertingTicker(merkutCtx context.Context) error {
	defer func() {
...
func (e *AlertingService) runJobDispatcher(merkutCtx context.Context) error {
	dispatcherGroup, alertCtx := errgroup.WithContext(merkutCtx)
...
func (e *AlertingService) processJobWithRetry(merkutCtx context.Context, job *Job) error {
	defer func() {
		if err := recover(); err != nil {
	...
	for {
		...
		case <-merkutCtx.Done():
			...
			case <-unfinishedWorkTimer.C:
				return e.endJob(merkutCtx.Err(), cancelChan, job)
			case <-attemptChan:
